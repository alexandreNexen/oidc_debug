...
export function renderFlowResultPage({ flow, serviceProvider, steps = [], flash, recommendedAction = "" }) {
  ...
  const detailsHref = `/legacy/oidc/flows/${encodeURIComponent(flow.id)}/details`;
  ...
  const body = `
    ...
    <div class="flow-actions">
      ...
      <form method="post" action="/legacy/oidc/flows/${encodeURIComponent(flow.id)}/rerun">
        ${renderIconBtn({ icon: "replay", label: "Run again", type: "submit", variant: "neutral", showLabel: true })}
      </form>
      ${failed && serviceProvider?.id ? renderIconBtn({ icon: "edit", label: "Edit Service Provider", href: `/legacy/oidc/service-providers/${encodeURIComponent(serviceProvider.id)}/edit`, variant: "neutral", showLabel: true }) : ""}
      ${renderIconBtn({ icon: "return", label: "Back to Service Providers", href: "/legacy/oidc/service-providers", variant: "neutral", showLabel: true })}
    </div>
  `;
