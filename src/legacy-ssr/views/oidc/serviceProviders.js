    ...
    <div class="card">
      <div class="card__body card__body--centered">
        <div class="empty-state">
          ...
          ${renderIconBtn({ icon: "add", href: "/legacy/oidc/service-providers/new", variant: "success", showLabel: true })}
        </div>
      </div>
    </div>
  `;
...
function renderLastFlow(lastFlow) {
  ...
  return `
    <a class="last-flow-link" href="/legacy/oidc/flows/${encodeURIComponent(lastFlow.id)}" aria-label="Open last OIDC flow, ${escapeHtml(label)}, started at ${escapeHtml(startedAt)}">
...
function renderEnvironmentPanel(env) {
  ...
  return `
    <details class="config-panel">
      ...
      <div class="config-panel__body">
        <form class="discovery-url-form" data-discovery-form data-env="${escapeHtml(envKey)}" action="/legacy/oidc/discovery/import/${escapeHtml(envKey)}" method="post">
          <div class="discovery-url-row">
            ...
            <button type="submit" class="btn-icon btn-icon--labeled btn-icon--neutral" data-discovery-submit>
              <img src="/legacy/assets/icons/replay.svg" width="16" height="16" alt="" aria-hidden="true">
              Validate &amp; import
            </button>
          </div>
      ...
      return `
        <tr>
          ...
          <td class="table__actions">
            ...
            ${renderIconBtn({ icon: "edit", label: "Edit", href: `/legacy/oidc/service-providers/${encodeURIComponent(sp.id)}/edit`, variant: "neutral" })}
            <form method="post" action="/legacy/oidc/service-providers/${encodeURIComponent(sp.id)}/delete" data-confirm="Delete this Service Provider?">
...
export function renderServiceProvidersPage({ serviceProviders = [], flash, fixedRedirectUri = "", oidcPageConfiguration = {} }) {
  ...
  const body = `
    ...
    ${renderPageHeader({
      ...
      actions: renderIconBtn({ icon: "add", href: "/legacy/oidc/service-providers/new", variant: "success", showLabel: true })
    })}
