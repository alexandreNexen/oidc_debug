...
async function copyToClipboard(text, button) {
  ...
  try {
    ...
    button.innerHTML = '<img src="/legacy/assets/icons/true.svg" width="16" height="16" alt="Copied" style="display:block">';
...
function setupDiscoveryForms() {
  document.querySelectorAll("[data-discovery-form]").forEach((form) => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        const response = await fetch(`/api/oidc/discovery/import/${encodeURIComponent(environmentKey)}`, {
          method: "POST",
