...
function normalizeStep(step = {}, { createId }) {
  ...
  return {
    ...
    flowId: step.flowId || "",
    stepName: SAML_STEP_ORDER.includes(step.stepName) ? step.stepName : "authn_request_created",
...
function sortSteps(entries = []) {
  return [...entries].sort((a, b) => {
    const orderDelta = SAML_STEP_ORDER.indexOf(a.stepName) - SAML_STEP_ORDER.indexOf(b.stepName);
    if (orderDelta !== 0) return orderDelta;
    return new Date(a.createdAt || 0).getTime() - new Date(b.createdAt || 0).getTime();
  });
}

export function createSamlFlowService({ getFlows, setFlows, getSteps, setSteps, createId, onChange = () => {} }) {
  function listFlows() {
  ...
  function hydrateSamlFlows(flows = [], steps = []) {
  ...
  function updateFlow(flowId, patch = {}) {
    ...
    const next = normalizeFlow({ ...existing, ...patch });
  ...
  function addFlowStep(flowId, step = {}) {
    const normalized = normalizeStep(
      { ...step, flowId, id: step.id || getFlowStep(flowId, step.stepName)?.id },
      { createId }
    );
