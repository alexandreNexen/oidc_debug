...
import { getOidcFlows, rerunOidcFlow } from "../api/client.js";
...
export default function OidcFlowsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const payload = await getOidcFlows();
    ...
    <div className="page">
      ...
      <main className="shell">
        <section className="page-header">
          <div>
            <h1 className="page-title">OIDC flows</h1>
            <p className="muted">
              Backed by <code className="code">/api/oidc/flows</code>.
            </p>
          </div>
        ...
        {state.status === "error" ? (
          <div className="alert alert--error" role="alert">
            <strong>Impossible de récupérer les flows OIDC.</strong>
        ...
        <Card
          title={`${items.length} OIDC flow${items.length === 1 ? "" : "s"} recorded`}
        >
          {items.length === 0 && state.status === "ready" ? (
            <p className="muted empty">Aucun flow OIDC enregistré.</p>
          ) : (
            <ul className="list">
              {items.map((flow) => (
                <li key={flow.id} className="list-row">
                  <div className="list-main">
                    <a
                      href={`/oidc/flows/${encodeURIComponent(flow.id)}`}
                    ...
                    <div className="list-meta muted">
                      ...
                      {flow.lastStep ? <span>· last step {fmt(flow.lastStep)}</span> : null}
                    </div>
                    ...
                    <div className="list-meta">
                      <FlowActionButton
                        ...
                        onAction={() => rerunOidcFlow(flow.id)}
        ...
        <footer className="footer">
          <p className="muted">
            Vue read-only. Aucun token, aucun secret n&apos;est exposé ici — seuls les
            identifiants de flow, timestamps et codes d&apos;erreur sont affichés.
          </p>
        </footer>
      </main>
    </div>
